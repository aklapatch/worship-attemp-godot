...
func _on_slides_display_texture(texture: String):
	...
	if self.self_modulate == Color.TRANSPARENT:
		...
		mod_tween.tween_property(self, "self_modulate", default_mod, .25).set_trans(Tween.TRANS_LINEAR)
	elif self.self_modulate == default_mod:
		# Only fade out
		mod_tween.tween_property(self, "self_modulate", Color.TRANSPARENT, .25).set_trans(Tween.TRANS_LINEAR)
	else:
